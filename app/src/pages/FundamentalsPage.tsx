...
export default function FundamentalsPage() {
  ...
  const [variants, setVariants] = useState<PersonalValuation[]>([]);
  ...
  useEffect(() => {
    let active = true;
    setVariants([]); setVariantsError(null); setVariantLabel('');
    if (sel?.automation && sel.automation.status !== 'unavailable') {
      personalVariants(sel.ticker).then(rows => { if (active) setVariants(rows); })
        .catch(error => { if (active) setVariantsError(String(error?.message || error)); });
    }
    return () => { active = false; };
  }, [sel?.ticker, sel?.automation?.status, revision]);
  ...
  const modelAction = async (kind: 'refresh' | 'lock' | 'variant') => {
    if (!sel || actionBusy) return;
    const model = sel;
    const generation = model.automation?.current?.generation_id;
    const key = JSON.stringify([model.ticker, kind, generation, kind === 'variant' ? variantLabel.trim() : null]);
    setActionBusy(true); setActionMessage(null);
    try {
      const requestId = pendingActions.current.get(key) || crypto.randomUUID();
      pendingActions.current.set(key, requestId);
      if (kind === 'refresh') {
        const job = await requestModelRefresh(model.ticker, requestId);
        setActionMessage(`${model.ticker}: ${tr('fundamentals.preparationState', { status: job.status, reason: job.reason || '' })}`);
      } else if (generation && kind === 'lock') {
        const locked = !model.automation?.locked;
        await lockModel(model.ticker, generation, locked);
        setActionMessage(`${model.ticker}: ${tr(locked ? 'fundamentals.modelLocked' : 'fundamentals.modelUnlocked')}`);
      } else if (generation && kind === 'variant') {
        const copy = await createPersonalVariant(model.ticker, generation, variantLabel.trim(), requestId);
        setActionMessage(`${model.ticker}: ${tr('fundamentals.variantState', { label: copy.label, status: copy.status })} ${copy.error || ''}`);
        if (copy.status === 'pending') return; // Retry the same reserved copy after a save failure.
      } else throw new Error(tr('fundamentals.currentUnavailable'));
      pendingActions.current.delete(key);
      setRevision(value => value + 1);
    } catch (error) {
      setActionMessage(`${model.ticker}: ${error instanceof Error ? error.message : String(error)}`);
    } finally { setActionBusy(false); }
  };

  const downloadCurrent = async (model: ValuationModel, historical = false) => {
    setDownloadError(null);
    try {
      const blob = await (historical ? historicalWorkbook(model) : currentWorkbook(model));
      saveBlob(blob, model.file);
    } catch (error) {
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-xs">
                ...
                <span>
                  ...
                  <span className="block text-[10px] text-faint">{marketQuote?.fx
                    ? tr('fundamentals.quoteFxBasis', { valuationDate: marketQuote.model_valuation_date ?? tr('fundamentals.f001'),
                      rate: fmtNum(marketQuote.fx.rate, 8), quoteCurrency: marketQuote.fx.quote_currency,
                      financialCurrency: marketQuote.fx.financial_currency, date: marketQuote.fx.on })
                    : tr('fundamentals.quoteNoRollforward')}</span>
                </span>
                {d ? (
                  <>
              ...
              <div className="flex gap-2 pt-1">
                {sel.current_download ? <button onClick={() => void downloadCurrent(sel)}
                ...
                </button> : sel.file && sel.historical_download && <button onClick={() => void downloadCurrent(sel, true)}
                   className="px-4 py-1.5 text-xs border border-[#7a5b1e] bg-[#1c1305] text-[#ffb000] hover:bg-[#33240a] inline-flex items-center gap-1.5">
                  <Download size={12} /> {tr('fundamentals.downloadHistorical')}
                </button>}
                {sel.memo_id && (
                  <span className="px-4 py-1.5 text-xs border border-[#2c4a7a] bg-[#0a1220] text-[#8ab4f8]">memo #{sel.memo_id} {tr('fundamentals.f134')}</span>
                )}
              </div>
              {sel.automation && sel.automation.status !== 'unavailable' && <div className="space-y-2 text-xs">
                <div className="flex flex-wrap gap-2">
                  <button disabled={actionBusy} onClick={() => void modelAction('refresh')}
                    className="px-3 py-1.5 border border-border text-amber disabled:opacity-50">{tr('fundamentals.requestRefresh')}</button>
                  {sel.automation.current && <button disabled={actionBusy} onClick={() => void modelAction('lock')}
                    className="px-3 py-1.5 border border-border text-amber disabled:opacity-50">{tr(sel.automation.locked ? 'fundamentals.unlockModel' : 'fundamentals.lockModel')}</button>}
                </div>
                {sel.current_download && <div className="flex flex-wrap gap-2">
                  <input aria-label={tr('fundamentals.variantLabel')} placeholder={tr('fundamentals.variantLabel')}
                    maxLength={120} value={variantLabel} onChange={event => setVariantLabel(event.target.value)}
                    className="px-2 py-1 bg-[#101010] border border-border text-white" />
                  <button disabled={actionBusy || !variantLabel.trim()} onClick={() => void modelAction('variant')}
                    className="px-3 py-1.5 border border-border text-amber disabled:opacity-50">{tr('fundamentals.createVariant')}</button>
                </div>}
                {variants.map(copy => <div key={copy.id} className="flex flex-wrap items-center gap-2 text-muted">
                  <span>{copy.label} · {displayedDate(copy.created_at)} · {copy.status}{copy.modified ? ` · ${tr('fundamentals.variantModified')}` : ''}</span>
                  {copy.available && <button className="text-amber" onClick={() => {
                    setDownloadError(null);
                    void personalWorkbook(copy).then(blob => saveBlob(blob, `PERSONAL_${copy.id}.xlsx`))
                      .catch(error => setDownloadError(String(error?.message || error)));
                  }}>{tr('fundamentals.openVariant')}</button>}
                </div>)}
                {variantsError && <p className="text-amber">{variantsError}</p>}
                {actionMessage && <p role="status" className="text-amber">{actionMessage}</p>}
              </div>}
              {sel.automation && <div className="text-xs text-muted space-y-1">
                <p>{sel.automation.status === 'unavailable' ? tr('fundamentals.registryUnavailable') : sel.automation.current ? tr('fundamentals.currentRevision', {
                  revision: sel.automation.current.revision,
                  date: displayedDate(sel.automation.current.published_at),
                  sources: displayedDate(sel.automation.current.as_of),
                }) : tr('fundamentals.noPublishedModel')}</p>
                <p>{sel.automation.approval?.active ? tr('fundamentals.pmApproved') : tr('fundamentals.notPmApproved')}
                  {sel.automation.locked ? ` · ${tr('fundamentals.modelLocked')}` : ''}</p>
                {sel.automation.latest_publication_attempt && sel.automation.latest_publication_attempt.status !== 'published' &&
                  <p className="text-amber">{tr('fundamentals.refreshNotPublished', {
                    date: displayedDate(sel.automation.latest_publication_attempt.created_at),
                    reason: sel.automation.latest_publication_attempt.reason,
                  })}</p>}
                {sel.automation.latest_prepare_job && !['succeeded', 'incomplete'].includes(sel.automation.latest_prepare_job.status) &&
                  <p className="text-amber">{tr('fundamentals.preparationState', {
                    status: sel.automation.latest_prepare_job.status, reason: sel.automation.latest_prepare_job.reason || '',
                  })}</p>}
                {sel.automation.latest_price_job && sel.automation.latest_price_job.status !== 'succeeded' &&
                  <p className="text-amber">{tr('fundamentals.priceRefreshFailed', {
                    date: displayedDate(sel.automation.latest_price_job.updated_at),
                    reason: sel.automation.latest_price_job.reason || sel.automation.latest_price_job.status,
                  })}</p>}
                <p>{tr('fundamentals.datedCopy')}</p>
              </div>}
              {downloadError && <p role="alert" className="text-crimson text-xs">{downloadError}</p>}
            </div>
          )}
        </div>
      </div>
            ...
            <div className="mt-2 space-y-1">
              {vecchi.map(f => (
                <div key={`${f.dir}/${f.file}/${f.generation_id || f.ticker}`} className="flex items-center justify-between text-xs py-1">
                  ...
                  {f.file && f.historical_download && <button onClick={() => void downloadCurrent(f, true)}
