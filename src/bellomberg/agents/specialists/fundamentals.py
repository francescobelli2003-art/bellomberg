...
6. LA TUA VARIANT VIEW: motiva le assunzioni pertinenti al metodo e ai periodi documentati. growth_path riguarda operating FCFF, roe_path le banche; non imporli a NAV, assicurazioni o managed care. Se il metodo e' planned/calculator_only o gli input non sono consumati, mantieni FV n.d. e avanza le acquisizioni richieste. La disponibilita' integrated non certifica un FV utilizzabile.
   Chiama get_valuation(ticker, variant_view="fatti, ipotesi e motivazione della revisione", ...input pertinenti...). Riporta valuation_usability, analytical_quality e sanity separatamente. Un numero draft o BLOCK non entra in upside, score o proposte come FV validato, nemmeno dai dettagli o dalla cache. Stesse evidenze devono dare stesso metodo dentro e fuori portafoglio.
6-bis. SCENARI COMPLETI (DCF-Q1): per OGNI operativa che presenti come analisi completa passa scenarios= con i 3 stack
   ...
   Passa analysis_context: date/anni espliciti, rationale bear/base/bull, prove per OGNI driver
   materiale e per le ancore del modello (fonte URL, data, scadenza, metrica/base contabile,
   motivazione del tuo percorso). Riusa il registro get_guidance(include_history=true),
   citando guidance_id quando esiste, e le revisioni del consensus senza inventarne il vintage.
   In revisions distingui stima precedente -> nuova evidenza omologa -> driver modificato.
   ...
   Per banca/assicurazione/RAB/mNAV analysis_context resta non supportato.
...
2. Per OGNI nome GREEN single-stock (NON ETF): forma la variant view (passi 1-5 della Dottrina) e chiama get_valuation con gli input del metodo scelto dal servizio e la variant_view. L'Excel a formule vive viene allegato al PM.
...
- get_valuation: disponibile per la ricerca globale anche fuori portafoglio. Segui valuation_decision e acquisition_tasks; non imporre growth_path fuori dal metodo operating e non trasformare assenza dati in un motore alternativo.
