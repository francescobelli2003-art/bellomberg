...
def chart_mc_fan(mc):
    ...
    try:
        ...
        ax.annotate(_t("NAV oggi  {:,.0f}k€").format(base / 1000), xy=(0, base),
        ...
        ax.set_xlim(0, max(d))
        ax.grid(axis="y")
        ax.set_xlabel(_t("Giorni di trading"))
        ...
        if counts and edges and len(edges) == len(counts) + 1:
            ...
            mx.grid(False)
            mx.set_title(_t("Orizzonte {}").format(_horizon_label(mc)), fontsize=6.8,
        ...
        sub = _t("{} · {:,} scenari · orizzonte {} gg").format(
            (mc.get("method_description") or mc.get("method", ""))[:58],
            mc.get("n_sims") or 0, mc.get("horizon_days", "?"))
        pneg = mc.get("prob_negative_pct")
        if pneg is not None:
            sub += _t(" · P(anno in perdita) {:.0f}%").format(pneg)
        st.titlebar(fig, _t("Proiezione NAV — Monte Carlo"), sub,
    ...
    try:
        ...
        if rets is not None and len(rets) > 30:
            ...
            ax.plot(range(n_hist), roll, color=st.NAVY, lw=1.3,
                    label=_t("Vol realizzata (22d ann.)"))
        ...
        ax.plot(xs, ys, color=st.GOLD, lw=1.8, marker="o", ms=3.5,
                label=_t("Forecast {}").format(garch.get("model_chosen", "GARCH")))
        ax.fill_between(xs, lo, hi, color=st.GOLD, alpha=0.18, label="CI 95%")
        if n_hist:
            ax.axvline(n_hist, color=st.MUTED, lw=0.8, ls=(0, (3, 3)))
        ax.set_ylabel(_t("Volatilita' annualizzata %"))
        ax.set_xlabel(_t("Giorni"))
        ax.grid(axis="y"); ax.tick_params(length=2)
        ax.legend(loc="upper left", fontsize=7.5, handlelength=1.4)
        st.titlebar(fig, _t("Volatilita' di portafoglio — realizzata e forecast GARCH"),
                    _t("Persistenza {} · vol prevista al giorno t+h (1/5/22g) · CI 95% simulati dal modello").format(
...
def chart_underwater(nav_hist):
    ...
    try:
        ...
        ax.set_ylabel(_t("Drawdown %"))
        ax.set_xlabel(_t("Giorni (dal primo trade)"))
        ax.grid(axis="y"); ax.tick_params(length=2)
        st.titlebar(fig, _t("Underwater chart — drawdown ITD"),
                    _t("Su cumulative P/L ratio: esclude l'effetto degli apporti di capitale"),
    ...
    try:
        ...
        if sd > 0:
            ...
            ax.plot(xs, np.exp(-(xs - mu) ** 2 / (2 * sd ** 2)) / (sd * np.sqrt(2 * np.pi)),
                    color=st.MUTED, lw=1.0, ls=(0, (4, 3)), label=_t("Normale equivalente"))
        ...
        ax.set_xlabel(_t("Rendimento giornaliero %"))
        ax.set_ylabel(_t("Densita'"))
        ax.grid(axis="y"); ax.tick_params(length=2)
...
def chart_corr_heatmap(corr_obj, risk_obj=None, n_total=None):
    ...
    try:
        ...
        est_lbl = (_t("Ledoit-Wolf (shrinkage a correlazione costante)")
                   if est.startswith("ledoit_wolf") else
                   _t("correlazione campionaria (Ledoit-Wolf fallito)") if est.startswith("sample") else
                   _t("estimatore n.d."))
        basis = _find_key(risk_obj or {}, "returns_basis") if risk_obj else None
        sub = _t("{} · rendimenti giornalieri in {}").format(est_lbl, basis or _t("base valutaria n.d."))
        obs = (meta or {}).get("obs")
        if obs:
            sub += _t(" · {} oss.").format(obs)
        ...
        if n_total and n_total > shown:
            sub += _t(" · {} nomi su {} posizioni").format(shown, n_total)
        elif n_tot > k:
            sub += _t(" · primi {} nomi su {} (leggibilita')").format(k, n_tot)
        st.titlebar(fig, _t("Matrice di correlazione"), sub,
...
def _numeric_tables(risk, mc, ff):
    ...
    if p.get("max_dd_1y_pct") is not None:
        add(_t("Max Drawdown (1 anno)"), _fmt(p["max_dd_1y_pct"], "%"),
            _t("massima caduta picco-minimo nell'ultimo anno"))
    if mcd.get("prob_negative_pct") is not None:
        add(_t("Prob. anno in perdita"), _fmt(mcd["prob_negative_pct"], "%"),
            _t("da Monte Carlo, probabilita' di chiudere l'anno sotto zero"))
    if len(rows) > 1:
    ...
    if p.get("beta_vs_spy") is not None:
        notes.append(_t("Con beta {} verso l'S&P 500, il book {} i movimenti del mercato USA.")
                     .format(_fmt(p["beta_vs_spy"]),
                             ...
                             _t("attenua") if p["beta_vs_spy"] < 0.9 else _t("segue circa 1:1")))
    ...
    if ph:
        flow.extend(_sec(_t("Decomposizione Fattoriale (Fama-French 5 + Momentum, regionale)"), h2, 0.8))
        frows = [[_t("Titolo"), _t("Regione"), "β Mkt", "β SMB", "β HML", _t("Alpha a."), "t-stat", "R²"]]
        ...
        if agg:
            frows.append([_t("AGGREGATO"), f"{_fmt((ff or {}).get('coverage_weight_pct'),'%',0)} cov",
        ...
        flow.append(Spacer(1, 0.15 * cm))
        flow.append(Paragraph(
            _t("quant.factor_interpretation", coverage=_fmt((ff or {}).get('coverage_weight_pct'),'%',0), significant=nsig, count=nan), body))
...
def build_quant_appendix_v2(blackboard=None, portfolio_data=None, macro_data=None,
    ...
    sections = [
        (_t("PORTAFOGLIO — COMPOSIZIONE E CONCENTRAZIONE"), [
            (treemap, _t("Figura 1. Allocazione per peso — scala navy, top holding in oro.")),
            (conc, _t("Figura 2. Top-10 posizioni vs soglia di concentrazione.")),
        ]),
        (_t("RISCHIO PROSPETTICO — MONTE CARLO E VOLATILITA'"), [
            (chart_mc_fan(mc), _t("Figura 3. Proiezione NAV con bande percentili (FHS).")),
            (chart_garch_cone(garch, nav), _t("Figura 4. Vol realizzata e forecast GARCH con CI 95%.")),
        ]),
        (_t("RISCHIO REALIZZATO — DRAWDOWN E CODE"), [
            (chart_underwater(nav), _t("Figura 5. Drawdown storici su cumulative P/L.")),
            (chart_var_distribution(nav, risk), _t("Figura 6. Distribuzione rendimenti con VaR/coda.")),
        ]),
        (_t("STRUTTURA — CORRELAZIONI E MACRO"), [
            (chart_corr_heatmap(corr_src, risk_obj=risk,
                                n_total=len((portfolio_data or {}).get("positions") or []) or None),
             _t("Figura 7. Correlazioni annotate (cluster visibili).")),
            (yieldc, _t("Figura 8. Curve dei rendimenti US/Germania/Giappone: struttura a "
                     "termine intera, oggi vs 1 mese e 1 anno fa [src: macro_rates].")),
            (credit, _t("Figura 9. Credito high yield europeo (OAS ICE BofA, proxy dichiarato "
    ...
    doc = SimpleDocTemplate(output_path, pagesize=A4,
                            ...
                            title=_t("Bellomberg Quantitative Appendix"),
